...
export function Problem() {
  ...
  return (
    <section id="vision" aria-labelledby="problem-title" className="w-full max-w-5xl 2xl:max-w-7xl py-12 md:py-20 lg:pb-32 px-6 flex flex-col items-center text-center mx-auto">
      <m.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-100px" }} variants={staggerContainer} className="w-full">
        ...
        <m.h2 id="problem-title" variants={staggerItem} className="text-3xl md:text-5xl font-medium mb-6 text-white/90">
          {t("title")}
        </m.h2>
        <m.p variants={staggerItem} className="text-lg md:text-xl text-white/50 leading-relaxed font-light mb-16 px-4 max-w-3xl mx-auto">
          {t("subtitle")}
        </m.p>

        <m.div variants={staggerItem} className="grid grid-cols-1 md:grid-cols-3 gap-6 text-left">
          <m.div
